...
export const CardGallery: React.FC<CardGalleryProps> = ({ cards, onSelectCard }) => {
    ...
    const sortedCards = useMemo(() => compositeSort(cards), [cards]);
    const visibleCards = useMemo(() => sortedCards.slice(0, visibleCount), [sortedCards, visibleCount]);
    const hasMore = visibleCount < sortedCards.length;
    const sections = useMemo(() => groupCards(visibleCards), [visibleCards]);
    ...
    const prevOutcomes = useRef<Record<string, string>>({});

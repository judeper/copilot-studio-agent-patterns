...
- `Sarah Chen <sarah.chen@example.com>;John Doe <john@example.com>` (multiple)
...
Use the Office 365 Outlook trigger's structured output: `triggerOutputs()?['body/from']` returns a JSON string. In some trigger versions it's a plain string, in others it's structured as `{"name": "Sarah Chen", "address": "sarah.chen@example.com"}`. The flow must handle both formats. Use an expression like:

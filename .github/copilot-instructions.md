...
2. **Copilot Studio Agent** — 18 agent prompts (10 original + 7 new: Router, Calendar, Task, Email Compose, Search, Validation, Delegation + Draft Refiner) organized in a MARL pipeline (Triage→Research→Scorer→DraftGen→Humanizer via Flow-level chaining). Shared prompt patterns in `prompts/patterns/`. Provisioned via `scripts/provision-copilot.ps1`. Knowledge sources support YAML-based conditional filtering via `triggerCondition` (Power Fx expressions) — see `docs/deployment-guide.md` § Knowledge Source Configuration and `src/knowledge-sources-example.yaml`.
3. **Dataverse** (9 tables: `AssistantCards`, `SenderProfile`, `BriefingSchedule`, `ErrorLog`, `EpisodicMemory`, `SemanticKnowledge`, `UserPersona`, `SkillRegistry`, `SemanticEpisodic`) persists results with ownership-based row-level security. `AssistantCards` includes `SNOOZED` and `PENDING_MANUAL` card statuses, `RESOLVED_EXTERNALLY` card outcome, and columns for snooze (`cr_snoozeduntil`), triage explainability (`cr_triagereasoning`), and focus protection (`cr_focusshieldactive`). `UserPersona` includes graduated autonomy columns (`cr_autonomytier`, `cr_totalinteractions`, `cr_acceptancerate`) and cold-start tone bootstrap (`cr_tonebaseline`).
4. **Canvas App + PCF React Dashboard** (PCF manifest v2.2.0, 233 tests across 16 suites) renders a single-pane-of-glass UI with WCAG AA compliance. The UX is grounded in cognitive science research (Cowan's 4±1 attention slots, Gloria Mark's 23-min interruption cost, Zeigarnik Effect, arXiv 2024 AI trust miscalibration, PMC visual fatigue). Key UX features: three-state confidence display (not percentages), 5-item focused queue with composite sort, quiet mode for focus protection, morning/EOD/meeting briefing variants via DayGlance component, warm-gray palette for sustained use, and `prefers-reduced-motion` support. Planned features: conversation threading (Gap 1), snooze/defer (Gap 2), batch actions (Gap 4), card search (Gap 11), keyboard navigation (Gap 12), undo toast (Gap 13) — see `.planning/design-review-productivity-noise.md`.
...
Design documents in `intelligent-work-layer/docs/`: `architecture-overview.md` (system architecture and positioning), `architecture-enhancements.md` (MARL pipeline design, signal batching, focus shield, deduplication, degraded mode, data retention), `learning-enhancements.md` (learning system design, graduated autonomy, cold-start tone bootstrap), `ux-enhancements.md` (UX improvements, WCAG AA compliance, card search, undo toast), `agent-contract.md` (Work OS agent-to-UI contract proposal). Design review: `.planning/design-review-productivity-noise.md` (17 gaps prioritized P0–P3).
...
The output JSON schema (`schemas/output-schema.json`), agent prompts (`prompts/`), TypeScript types (`src/AssistantDashboard/components/types.ts`), and Dataverse table definitions (`schemas/dataverse-table.json`) must all stay in sync. When changing a field, update all four locations. Recent additions: `triage_reasoning` (Gap 9) and `conversation_cluster_action` (Gap 1) fields span all four artifacts. New Dataverse columns (`cr_snoozeduntil`, `cr_triagereasoning`, `cr_focusshieldactive`) and UserPersona columns (`cr_autonomytier`, `cr_totalinteractions`, `cr_acceptancerate`, `cr_tonebaseline`) follow the same contract.

...
1. **Jest test suite:** `cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --verbose`
   - Expected: 150 tests pass across 11 suites, 0 failures

2. **TypeScript type-check:** `cd intelligent-work-layer/src && npx tsc --noEmit`
   - Expected: 0 errors

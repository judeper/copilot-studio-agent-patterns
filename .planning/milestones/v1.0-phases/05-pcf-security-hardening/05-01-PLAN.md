...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/src/AssistantDashboard/utils/urlSanitizer.ts"
  ...
  key_links:
    - from: "intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx"
      to: "intelligent-work-layer/src/AssistantDashboard/utils/urlSanitizer.ts"
...
<task type="auto">
  ...
  <files>intelligent-work-layer/src/AssistantDashboard/utils/urlSanitizer.ts</files>
  <action>
Create a new file `intelligent-work-layer/src/AssistantDashboard/utils/urlSanitizer.ts` with:
...
Do NOT use regex for URL parsing. Do NOT use a blocklist approach. Do NOT add any npm dependencies.
  ...
  <verify>
Run `bun run build` from `intelligent-work-layer/` -- must compile with zero errors. Verify the file exists and exports `isSafeUrl` and `SAFE_PROTOCOLS` by checking the build output.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx</files>
  <action>
...
- Keep `target="_blank"` and `rel="noopener noreferrer"` on the Link element (already correct)
  ...
  <verify>
1. Run `bun run build` from `intelligent-work-layer/` -- zero errors, zero warnings
2. Run `bun run lint` from `intelligent-work-layer/` -- zero errors, zero warnings

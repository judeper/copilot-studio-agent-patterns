...
must_haves:
  ...
  artifacts:
    ...
    - path: "intelligent-work-layer/src/AssistantDashboard/components/FilterBar.tsx"
      provides: "Consolidated Fluent UI imports from react-components"
      contains: "tokens"
    - path: "intelligent-work-layer/src/AssistantDashboard/components/CardGallery.tsx"
      provides: "Consolidated Fluent UI imports from react-components"
      contains: "tokens"
    ...
    - path: "intelligent-work-layer/src/.eslintrc.json"
      ...
      contains: "no-console"
    - path: "intelligent-work-layer/src/bun.lockb"
      provides: "Deterministic Bun lockfile for reproducible installs"
  key_links:
    - from: "intelligent-work-layer/src/AssistantDashboard/ControlManifest.Input.xml"
    ...
    - from: "intelligent-work-layer/src/AssistantDashboard/components/FilterBar.tsx"
      to: "@fluentui/react-components"
      via: "import statement for tokens"
      pattern: 'import.*tokens.*from.*@fluentui/react-components'
    - from: "intelligent-work-layer/src/AssistantDashboard/components/CardGallery.tsx"
      to: "@fluentui/react-components"
      via: "import statement for tokens"
      pattern: 'import.*tokens.*from.*@fluentui/react-components'
---
...
Run `grep -r "@fluentui/react-theme" intelligent-work-layer/src/` -- must return zero results.
  </action>
  <verify>
- `grep -r "@fluentui/react-theme" intelligent-work-layer/src/` returns no results
- `grep 'version="9.46.2"' intelligent-work-layer/src/AssistantDashboard/ControlManifest.Input.xml` confirms manifest version
...
<task type="auto">
  ...
  <files>
    intelligent-work-layer/src/.eslintrc.json
    intelligent-work-layer/src/package.json
    intelligent-work-layer/src/bun.lockb
  </files>
  <action>
...
Run from `intelligent-work-layer/src/`:
```bash
bun install
```
...
- `ls intelligent-work-layer/src/bun.lockb` confirms lockfile exists
- `git ls-files intelligent-work-layer/src/bun.lockb` confirms lockfile is tracked (after commit)
- `grep '"@fluentui/react-components": "\\^9.46.0"' intelligent-work-layer/src/package.json` confirms version pin unchanged
...
<verification>
1. `grep -r "@fluentui/react-theme" intelligent-work-layer/src/` returns zero results
2. `grep 'version="9.46.2"' intelligent-work-layer/src/AssistantDashboard/ControlManifest.Input.xml` matches
3. `bun run build` in `intelligent-work-layer/src/` exits 0 with clean output
4. `bun run lint` in `intelligent-work-layer/src/` exits 0 with zero warnings

...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/scripts/audit-table-naming.ps1"
      provides: "Reusable naming convention audit script"
      contains: "cr_assistantcard"
  key_links:
    - from: "intelligent-work-layer/scripts/audit-table-naming.ps1"
      to: "all files in intelligent-work-layer/"
...
Output: `intelligent-work-layer/scripts/audit-table-naming.ps1` -- a PowerShell script that reports violations and correct usages, with non-zero exit code on violations.
</objective>
...
<task type="auto">
  ...
  <files>intelligent-work-layer/scripts/audit-table-naming.ps1</files>
  <action>
Create `intelligent-work-layer/scripts/audit-table-naming.ps1` following the existing script conventions from `provision-environment.ps1` and `create-security-roles.ps1`:
...
- The publisher prefix is `cr` per project convention, but the script should work with the full `cr_assistantcard` pattern, not try to parameterize the prefix (the audit is specifically for this table's naming).
  ...
  <verify>
Run `pwsh -File intelligent-work-layer/scripts/audit-table-naming.ps1 -SearchRoot intelligent-work-layer/` (or equivalent) and confirm the script:
...
<task type="auto">
  ...
  <files>intelligent-work-layer/scripts/audit-table-naming.ps1</files>
  <action>
Run the audit script created in Task 1 against the full `intelligent-work-layer/` directory:

```bash
cd intelligent-work-layer && pwsh -File scripts/audit-table-naming.ps1 -SearchRoot .
```
...
<verification>
1. `intelligent-work-layer/scripts/audit-table-naming.ps1` exists and follows project PowerShell conventions
2. Running `pwsh -File intelligent-work-layer/scripts/audit-table-naming.ps1 -SearchRoot intelligent-work-layer/` produces zero violations

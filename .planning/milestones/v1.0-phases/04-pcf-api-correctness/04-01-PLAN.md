...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/src/AssistantDashboard/components/types.ts"
      provides: "Priority and TemporalHorizon types without N/A, nullable fields in AssistantCard"
      contains: "priority: Priority | null"
    - path: "intelligent-work-layer/src/AssistantDashboard/components/constants.ts"
    ...
    - path: "intelligent-work-layer/src/AssistantDashboard/hooks/useCardData.ts"
      ...
      contains: "?? null"
    - path: "intelligent-work-layer/src/AssistantDashboard/components/CardItem.tsx"
...
3. Change line 46 item_summary from `String(parsed.item_summary ?? record.getValue("cr_itemsummary") ?? "")` to:
  ...
  <verify>
Run `cd intelligent-work-layer/src && npx tsc --noEmit` to verify type consistency. types.ts should compile without errors. useCardData.ts should compile without errors (the null assignments match the new `Priority | null` and `TemporalHorizon | null` types). constants.ts should export PRIORITY_COLORS without errors.
  </verify>
  <done>
...
```bash
cd intelligent-work-layer/src && bun run build && bun run lint
```
...
Per 04-RESEARCH.md, all Badge size values (small, medium), all Badge appearance values (filled, outline, tint), all Badge color values (success, warning, informative, subtle), and all token references are already valid against the Fluent UI v9 API surface. No further prop changes needed. This task confirms the audit finding by achieving a clean build.
  ...
  <verify>
1. `cd intelligent-work-layer/src && bun run build` exits with code 0 and zero errors
2. `cd intelligent-work-layer/src && bun run lint` exits with code 0 and zero warnings
3. `grep -r '"N/A"' intelligent-work-layer/src/AssistantDashboard/` returns no matches (no residual N/A string references in any component or hook)
4. `grep -r 'priorityColors' intelligent-work-layer/src/AssistantDashboard/` returns no matches (local maps fully removed)
5. `grep -r 'PRIORITY_COLORS' intelligent-work-layer/src/AssistantDashboard/` returns matches only in constants.ts, CardItem.tsx, and CardDetail.tsx
6. `grep -r 'No summary available' intelligent-work-layer/src/AssistantDashboard/` returns no matches
  </verify>
  <done>
...
4. **No N/A residue:** `grep -rn '"N/A"' intelligent-work-layer/src/AssistantDashboard/` returns zero matches
5. **No duplicate maps:** `grep -rn 'priorityColors' intelligent-work-layer/src/AssistantDashboard/` returns zero matches
6. **Shared constants used:** `grep -rn 'PRIORITY_COLORS' intelligent-work-layer/src/AssistantDashboard/` shows imports in CardItem.tsx and CardDetail.tsx, definition in constants.ts

...
must_haves:
  truths:
    - "npx jest --passWithNoTests runs without configuration errors from the intelligent-work-layer/src/ directory"
...
@intelligent-work-layer/src/package.json
@intelligent-work-layer/src/tsconfig.json
...
<task type="auto">
  ...
  <action>
    **Step 1 — Install dependencies.** From `intelligent-work-layer/src/`, run:
    ```bash
    ...
    **Step 2 — Create `tsconfig.test.json`** in `intelligent-work-layer/src/`. This extends the main tsconfig but overrides `module` to CommonJS for Jest compatibility and adds test-specific types:
    ```json
    {
    ...
    **Step 3 — Create `test/jest.config.ts`:**
    - `rootDir: '..'` (points to `intelligent-work-layer/src/`)
  ...
  <verify>
    From `intelligent-work-layer/src/`, run:
    ```bash
...
<verification>
1. `cd intelligent-work-layer/src && npx jest --passWithNoTests` exits 0

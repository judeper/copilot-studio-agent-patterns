...
|-------------------------------------------------------------------------------------|-------------------------------------------------------|------------|---------------------------------------------------------------------------------|
| `intelligent-work-layer/src/test/jest.config.ts`                                 | Jest configuration with ts-jest, jsdom, thresholds    | VERIFIED   | 51 lines; contains `testEnvironment`, `ts-jest`, `setupFilesAfterEnv`, `coverageThreshold` |
| `intelligent-work-layer/src/test/jest.setup.ts`                                  | jest-dom matchers and matchMedia polyfill             | VERIFIED   | 27 lines; imports `@testing-library/jest-dom`; contains `matchMedia` mock and `ResizeObserver` mock |
| `intelligent-work-layer/src/tsconfig.test.json`                                  | Test-specific TypeScript config with CommonJS         | VERIFIED   | Contains `"module": "CommonJS"` and `"skipLibCheck": true`                      |
| `intelligent-work-layer/src/test/mocks/componentFramework.ts`                    | Factory function for creating mock PCF datasets       | VERIFIED   | Exports `createMockDataset` and `MockRecordData`; 35 lines with JSDoc           |
| `intelligent-work-layer/src/test/helpers/renderWithProviders.tsx`                | FluentProvider wrapper for component tests            | VERIFIED   | Exports `renderWithProviders`; 26 lines wrapping in `FluentProvider` + `webLightTheme` |
| `intelligent-work-layer/src/test/fixtures/cardFixtures.ts`                       | Shared fixtures for all triage tiers + edge cases     | VERIFIED   | Exports `tier1SkipItem`, `tier2LightItem`, `tier3FullItem`, plus 4 edge cases; 187 lines |
| `intelligent-work-layer/src/AssistantDashboard/hooks/__tests__/useCardData.test.ts`      | Hook tests — JSON parsing, edge cases, tiers          | VERIFIED   | 176 lines; 11 tests covering all required scenarios                             |
| `intelligent-work-layer/src/AssistantDashboard/utils/__tests__/urlSanitizer.test.ts`     | URL protocol validation tests                         | VERIFIED   | 55 lines; 12 tests covering https, mailto, javascript, data, null/undefined/empty |
| `intelligent-work-layer/src/AssistantDashboard/components/__tests__/App.test.tsx`        | Filter logic tests via rendered App                   | VERIFIED   | 169 lines; 10 tests (8 filter logic + 2 navigation)                             |
| `intelligent-work-layer/src/AssistantDashboard/components/__tests__/CardItem.test.tsx`   | CardItem render and click tests                       | VERIFIED   | 59 lines; 6 tests                                                               |
| `intelligent-work-layer/src/AssistantDashboard/components/__tests__/CardDetail.test.tsx` | CardDetail section rendering tests                    | VERIFIED   | 193 lines; 21 tests                                                             |
| `intelligent-work-layer/src/AssistantDashboard/components/__tests__/CardGallery.test.tsx`| CardGallery rendering and empty state                 | VERIFIED   | 42 lines; 3 tests                                                               |
| `intelligent-work-layer/src/AssistantDashboard/components/__tests__/FilterBar.test.tsx`  | FilterBar count and badge display                     | VERIFIED   | 81 lines; 5 tests                                                               |

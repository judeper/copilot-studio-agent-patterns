...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/schemas/output-schema.json"
    ...
    - path: ".planning/PROJECT.md"
      ...
      contains: "Execute Agent and wait"
  key_links:
    - from: "intelligent-work-layer/schemas/output-schema.json"
      to: "intelligent-work-layer/src/AssistantDashboard/components/types.ts"
...
<task type="auto">
  ...
  <action>
    **Fix 1 — Schema enum convention (output-schema.json):**
    ...
    In `intelligent-work-layer/schemas/output-schema.json`, update the `priority` field (around lines 37-39):
    ...
    **Fix 2 — Broken relative path (agent-flows.md):**
    ...
    In `intelligent-work-layer/docs/agent-flows.md` (around line 58), change:
    - `../../schemas/output-schema.json` to `../schemas/output-schema.json`

    The current path goes up two levels (to repo root) where no schemas/ directory exists. One `../` from docs/ reaches intelligent-work-layer/ which contains schemas/.

    **Verification:**
    - Run `node -e "console.log(JSON.parse(require('fs').readFileSync('intelligent-work-layer/schemas/output-schema.json','utf8')).properties.priority)"` to validate JSON syntax and confirm priority field structure.
    - Run `node -e "const p=require('path');const f='intelligent-work-layer/docs';console.log(require('fs').existsSync(p.resolve(f,'../schemas/output-schema.json')))"` to verify the relative path resolves to an existing file.
    - Confirm alignment with types.ts by reading the Priority and TemporalHorizon types.
  </action>
  <verify>
    1. `node -e "JSON.parse(require('fs').readFileSync('intelligent-work-layer/schemas/output-schema.json','utf8'));console.log('JSON valid')"` prints "JSON valid"
    2. `node -e "const s=JSON.parse(require('fs').readFileSync('intelligent-work-layer/schemas/output-schema.json','utf8'));const p=s.properties.priority;console.log(p.type,p.enum)"` shows `[ 'string', 'null' ] [ 'High', 'Medium', 'Low', null ]`
    3. `node -e "const p=require('path');const f='intelligent-work-layer/docs';console.log(require('fs').existsSync(p.resolve(f,'../schemas/output-schema.json')))"` prints `true`
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    **Fix 3 — Bun version annotation (deployment-guide.md):**
    ...
    In `intelligent-work-layer/docs/deployment-guide.md` (around line 9), change:
  ...
  <verify>
    1. `grep "Tested with Bun 1.3.8" intelligent-work-layer/docs/deployment-guide.md` returns a match

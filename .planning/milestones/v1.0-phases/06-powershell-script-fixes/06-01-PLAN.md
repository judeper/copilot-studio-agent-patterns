...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/scripts/deploy-solution.ps1"
  ...
  key_links:
    - from: "intelligent-work-layer/scripts/deploy-solution.ps1"
    ...
    - from: "intelligent-work-layer/docs/deployment-guide.md"
      to: "intelligent-work-layer/scripts/deploy-solution.ps1"
...
<task type="auto">
  ...
  <files>intelligent-work-layer/scripts/deploy-solution.ps1</files>
  <action>
...
- The backtick line continuation on `pac solution import` is preserved or reformatted to a single line
  ...
  <verify>
1. PowerShell syntax check: `pwsh -Command "& { Get-Command -Syntax (Resolve-Path 'intelligent-work-layer/scripts/deploy-solution.ps1') }" 2>&1` should not error

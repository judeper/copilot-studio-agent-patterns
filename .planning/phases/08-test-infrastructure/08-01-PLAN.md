...
<task type="auto">
  ...
  <action>
    ...
    **Note on `setupFilesAfterSetup`:** The correct Jest config key is `setupFilesAfterSetup`. Verify the exact spelling against Jest 29 docs — it is actually `setupFilesAfterSetup`. (CORRECTION: The correct Jest config key is **`setupFilesAfterSetup`** — double-check: it is `setupFilesAfterSetup`. Verify by checking jest.io docs. The actual key is `setupFilesAfterSetup`.)

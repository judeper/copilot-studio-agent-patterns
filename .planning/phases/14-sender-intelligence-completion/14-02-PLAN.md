...
must_haves:
  ...
  key_links:
    - from: "intelligent-work-layer/docs/agent-flows.md (Flow 1/2/3 trigger flows)"
      to: "intelligent-work-layer/prompts/main-agent-system-prompt.md"
      via: "SENDER_PROFILE input variable passed to Execute Agent action"
      pattern: "SENDER_PROFILE"
    - from: "intelligent-work-layer/docs/agent-flows.md (Flow 5 Branch A)"
...
<task type="auto">
  ...
  <verify>
    <automated>grep -c "Known Gap: R-17" intelligent-work-layer/docs/agent-flows.md | xargs test 0 -eq && grep -c "Upsert" intelligent-work-layer/docs/agent-flows.md | xargs test 0 -lt && grep -c "SENDER_PROFILE" intelligent-work-layer/docs/agent-flows.md | xargs test 5 -lt && echo "PASS" || echo "FAIL"</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <verify>
    <automated>grep -c "List+Condition+Add/Update\|List → Condition → Add/Update" intelligent-work-layer/docs/agent-flows.md | xargs test 0 -eq && grep -c "alternate key" intelligent-work-layer/docs/agent-flows.md | xargs test 2 -lt && grep -c "editDistanceRatio\|edit distance ratio" intelligent-work-layer/docs/agent-flows.md | xargs test 0 -lt && echo "PASS" || echo "FAIL"</automated>
  </verify>
  <done>
...
<verification>
1. `grep -c "Known Gap: R-17" intelligent-work-layer/docs/agent-flows.md` returns 0 — gap is resolved and removed
2. `grep -c "Upsert" intelligent-work-layer/docs/agent-flows.md` returns 4+ — upsert used in Flows 1, 2, 3, and 5
3. `grep -c "SENDER_PROFILE" intelligent-work-layer/docs/agent-flows.md` returns 10+ — passthrough documented in all trigger flows
4. `grep "cr_senderemail_key" intelligent-work-layer/docs/agent-flows.md` returns matches — alternate key referenced in upsert actions
5. `grep "editDistanceRatio" intelligent-work-layer/docs/agent-flows.md` returns match — pre-computed ratio referenced in Flow 5
6. `grep "Optional (Sprint 4)" intelligent-work-layer/docs/deployment-guide.md` returns 0 — Sprint 4 qualifier removed from SENDER_PROFILE

...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/src/.eslintrc.json"
      provides: "ESLint config with react-hooks plugin"
      contains: "react-hooks/rules-of-hooks"
    - path: "intelligent-work-layer/src/AssistantDashboard/utils/levenshtein.ts"
      provides: "Levenshtein edit distance computation"
      exports: ["levenshteinRatio"]
    - path: "intelligent-work-layer/src/AssistantDashboard/utils/__tests__/levenshtein.test.ts"
      provides: "Unit tests for Levenshtein utility"
      min_lines: 20
    - path: "intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx"
      provides: "Edit distance computation on send"
      contains: "levenshteinRatio"
    - path: "intelligent-work-layer/src/AssistantDashboard/index.ts"
      provides: "PCF output with editDistanceRatio in sendDraftAction JSON"
      contains: "editDistanceRatio"
  key_links:
    - from: "intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx"
      to: "intelligent-work-layer/src/AssistantDashboard/utils/levenshtein.ts"
      via: "import levenshteinRatio"
      pattern: "import.*levenshteinRatio.*from.*levenshtein"
    - from: "intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx"
      to: "intelligent-work-layer/src/AssistantDashboard/index.ts"
...
From intelligent-work-layer/src/AssistantDashboard/components/types.ts:
```typescript
...
From intelligent-work-layer/src/AssistantDashboard/index.ts:
```typescript
...
Current ESLint config (intelligent-work-layer/src/.eslintrc.json):
```json
{
...
<task type="auto" tdd="true">
  ...
  <action>
    Create `intelligent-work-layer/src/AssistantDashboard/utils/levenshtein.ts` with:
    ...
    Create the test file at `intelligent-work-layer/src/AssistantDashboard/utils/__tests__/levenshtein.test.ts` with the behaviors above.

    Follow TDD: write tests first, run to confirm failure, then implement, run to confirm passing.
  </action>
  <verify>
    <automated>cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --testPathPattern="levenshtein" --no-coverage</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    **Part A: ESLint react-hooks plugin setup**
    ...
    1. Install: `cd intelligent-work-layer/src && npm install --save-dev eslint-plugin-react-hooks@^4.6.0`
  ...
  <verify>
    <automated>cd intelligent-work-layer/src && npm run lint 2>&1 | grep -c "react-hooks" | xargs test 0 -eq && echo "PASS: zero react-hooks errors" || echo "FAIL: react-hooks errors remain"</automated>
  </verify>
  <done>
...
<verification>
1. `cd intelligent-work-layer/src && npm run lint` — exits 0 with no react-hooks errors
2. `cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --no-coverage` — all tests pass including levenshtein
3. Grep check: `grep -r "eslint-disable.*react-hooks" intelligent-work-layer/src/AssistantDashboard/` returns no results (no suppressed violations)
4. Grep check: `grep "editDistanceRatio" intelligent-work-layer/src/AssistantDashboard/index.ts` confirms ratio is in output payload
5. Grep check: `grep "levenshteinRatio" intelligent-work-layer/src/AssistantDashboard/components/CardDetail.tsx` confirms computation happens on send
</verification>

...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/src/AssistantDashboard/components/BriefingCard.tsx"
      provides: "Fluent UI BriefingCard with Back button"
      contains: "from \"@fluentui/react-components\""
    - path: "intelligent-work-layer/src/AssistantDashboard/components/ConfidenceCalibration.tsx"
...
1. All `<button>` elements → `<Button>` from Fluent UI:
   - "Open card →" links: `<Button appearance="transparent" size="small" icon={<ArrowRightRegular />} onClick={...}>Open card</Button>`
   - "Dismiss briefing" button: `<Button appearance="subtle" icon={<DismissRegular />} onClick={...}>Dismiss briefing</Button>`
   - "Open →" in StaleAlert: `<Button appearance="transparent" size="small" icon={<ArrowRightRegular />} onClick={...}>Open</Button>`
...
2. All `<h2>` / `<h3>` elements → `<Text>` with appropriate size/weight:
   - `<h2>Daily Briefing</h2>` → `<Text as="h2" size={500} weight="semibold" block>Daily Briefing</Text>`
   - `<h3>Overdue Items</h3>` → `<Text as="h3" size={400} weight="semibold" block>Overdue Items</Text>`
   - `<h3>Action Items</h3>` → `<Text as="h3" size={400} weight="semibold" block>Action Items</Text>`

3. Status/rank indicators → `<Badge>`:
   - Rank number `#{item.rank}` → `<Badge appearance="filled" size="small">#{item.rank}</Badge>`
   - FYI category → `<Badge appearance="outline" size="small">{categoryLabel}</Badge>`
   - Stale hours → `<Badge appearance="tint" color="warning" size="small">{hours}h pending</Badge>`

4. Descriptive `<span>` / `<p>` elements → `<Text>`:
   - Error message → `<Text block>Unable to parse...</Text>`
   - Day shape narrative → `<Text block size={300}>{briefing.day_shape}</Text>`
   - Date/count spans → `<Text size={200}>{briefing.briefing_date}</Text>` and `<Badge appearance="outline" size="small">{count} open items</Badge>`

5. Calendar icon text "📅" → `<CalendarRegular />` from `@fluentui/react-icons`
...
- Add a test: "renders Back button when onBack is provided" — render with `onBack={mockBack}`, click Back, assert `mockBack` was called.
- Add a test: "does not render Back button when onBack is omitted" — render without `onBack`, assert no Back button exists.
- Update existing text assertions if the rendered text changes (e.g., button text no longer includes "→" arrow in text content since it moves to an icon).
  </action>
  <verify>
    <automated>cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --testPathPattern="BriefingCard" --no-coverage 2>&1 | tail -20</automated>
  </verify>
...
1. **Tab bar** — Replace the manual `<div className="calibration-tabs">` with `<TabList>` + `<Tab>`:
   ```typescript
...
2. **Back button** — Replace `<button className="calibration-back" onClick={onBack}>← Back to Dashboard</button>` with:
   ```typescript
   <Button appearance="subtle" icon={<ArrowLeftRegular />} onClick={onBack}>Back to Dashboard</Button>
   ```

3. **Header text** — Replace `<h2>Agent Performance</h2>` with `<Text as="h2" size={500} weight="semibold" block>Agent Performance</Text>`

4. **Stat cards** — Replace `<div className="calibration-stat-card">` pattern with `<Card>` from Fluent UI (import it). Each stat card:
   ```typescript
...
5. **Accuracy rate badges** — Replace `<span className="calibration-good/ok/poor">` with `<Badge>`:
   ```typescript
   <Badge appearance="filled" color={rate >= 70 ? "success" : rate >= 40 ? "warning" : "danger"} size="small">{rate}%</Badge>
   ```

6. **Table elements** — Keep `<table>`, `<thead>`, `<tbody>`, `<tr>`, `<th>`, `<td>` as-is. Fluent UI v9 does not have a Table component that maps 1:1 to these; the HTML table elements are acceptable for tabular data display. Apply Fluent tokens for styling where inline styles exist.

7. **Description paragraphs** — Replace `<p className="calibration-description">` with `<Text block size={300} style={{ color: tokens.colorNeutralForeground2 }}>`.

8. **Sample count** — Replace `<span className="calibration-sample">` with `<Badge appearance="outline" size="small">`.

9. **UIUX-06 — Empty analytics buckets:**
   ...
   - `triageStats.fullTotal === 0` → show "No data" instead of "0%" for FULL card action rate
   - `triageStats.lightTotal === 0` → show "No data" instead of "0%" for LIGHT card dismiss rate
   - `draftStats.total === 0` → show "No data" instead of "0%" for sent-as-is rate
...
- The test does NOT mock Fluent UI (it renders real components). Update assertions for text changes:
  - "← Back to Dashboard" becomes just "Back to Dashboard" (arrow is now an icon)
  - Add test: "shows 'No data' instead of 0% for empty accuracy buckets" — render with 0 cards, verify "No data" text appears in the accuracy tab instead of "0%"
  ...
  <verify>
    <automated>cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --testPathPattern="ConfidenceCalibration" --no-coverage 2>&1 | tail -20</automated>
  </verify>
...
<verification>
1. Run all tests: `cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --no-coverage`
2. Verify zero plain HTML `<button>` elements in BriefingCard.tsx: `grep -c '<button' intelligent-work-layer/src/AssistantDashboard/components/BriefingCard.tsx` returns 0
3. Verify zero plain HTML `<button>` elements in ConfidenceCalibration.tsx: `grep -c '<button' intelligent-work-layer/src/AssistantDashboard/components/ConfidenceCalibration.tsx` returns 0
4. Verify BriefingCard has onBack prop: `grep 'onBack' intelligent-work-layer/src/AssistantDashboard/components/BriefingCard.tsx` finds the prop
5. Verify "No data" text exists in ConfidenceCalibration: `grep 'No data' intelligent-work-layer/src/AssistantDashboard/components/ConfidenceCalibration.tsx` finds matches
6. TypeScript compiles: `cd intelligent-work-layer/src && npx tsc --noEmit` passes
</verification>

<success_criteria>
- BriefingCard.tsx has zero plain HTML interactive elements (button, a, input) — all replaced with Fluent UI
- ConfidenceCalibration.tsx has zero plain HTML button elements — all replaced with Fluent UI

...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/schemas/dataverse-table.json"
      provides: "cr_reminderdue DateTime column definition for reminders"
      contains: "cr_reminderdue"
    - path: "intelligent-work-layer/prompts/orchestrator-agent-prompt.md"
    ...
    - path: "intelligent-work-layer/docs/agent-flows.md"
      ...
      contains: "Flow 10"
  key_links:
    - from: "intelligent-work-layer/docs/agent-flows.md (Flow 10)"
      to: "intelligent-work-layer/schemas/dataverse-table.json (cr_reminderdue)"
      via: "OData filter on cr_reminderdue le utcNow()"
      pattern: "cr_reminderdue"
    - from: "intelligent-work-layer/docs/agent-flows.md (Trigger Type Compose)"
      to: "intelligent-work-layer/schemas/dataverse-table.json (cr_triggertype options)"
...
<interfaces>
From intelligent-work-layer/schemas/dataverse-table.json — existing cr_triggertype Choice options:
- EMAIL → 100000000
...
From intelligent-work-layer/docs/agent-flows.md — current Trigger Type Compose (line ~261):
```
...
From intelligent-work-layer/prompts/orchestrator-agent-prompt.md — CreateCard required fields (line ~86-91):
...
From intelligent-work-layer/docs/agent-flows.md — existing flow numbering:
...
<task type="auto">
  ...
  <verify>
    <automated>grep -c "cr_reminderdue" intelligent-work-layer/schemas/dataverse-table.json intelligent-work-layer/prompts/orchestrator-agent-prompt.md</automated>
  </verify>
...
<task type="auto">
  ...
  <verify>
    <automated>grep -c "DAILY_BRIEFING.*100000003" intelligent-work-layer/docs/agent-flows.md && grep -c "Flow 10" intelligent-work-layer/docs/agent-flows.md && grep -c "cr_reminderdue" intelligent-work-layer/docs/agent-flows.md</automated>
  </verify>
...
<verification>
1. `grep "cr_reminderdue" intelligent-work-layer/schemas/dataverse-table.json` — column defined in Dataverse schema
2. `grep "cr_reminderdue" intelligent-work-layer/prompts/orchestrator-agent-prompt.md` — required field in orchestrator prompt
3. `grep "DAILY_BRIEFING.*100000003" intelligent-work-layer/docs/agent-flows.md` — Trigger Type Compose maps DAILY_BRIEFING
4. `grep "SELF_REMINDER.*100000004" intelligent-work-layer/docs/agent-flows.md` — Trigger Type Compose maps SELF_REMINDER
5. `grep "COMMAND_RESULT.*100000005" intelligent-work-layer/docs/agent-flows.md` — Trigger Type Compose maps COMMAND_RESULT
6. `grep "Flow 10" intelligent-work-layer/docs/agent-flows.md` — Flow 10 section exists
7. `grep "ten Power Automate flows" intelligent-work-layer/docs/agent-flows.md` — intro paragraph updated
</verification>

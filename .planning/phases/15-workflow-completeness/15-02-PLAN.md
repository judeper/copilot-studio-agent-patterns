...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/schemas/briefingschedule-table.json"
      provides: "BriefingSchedule Dataverse table definition"
      contains: "cr_briefingschedule"
    - path: "intelligent-work-layer/docs/agent-flows.md"
      provides: "Updated Flow 6 that reads schedule from Dataverse"
      contains: "BriefingSchedule"
    - path: "intelligent-work-layer/docs/canvas-app-setup.md"
      provides: "Briefing schedule configuration UI instructions"
      contains: "BriefingSchedule"
    - path: "intelligent-work-layer/docs/deployment-guide.md"
      provides: "Updated deployment guidance referencing BriefingSchedule table"
      contains: "BriefingSchedule"
    - path: "intelligent-work-layer/scripts/provision-environment.ps1"
      provides: "BriefingSchedule table creation commands"
      contains: "cr_briefingschedule"
  key_links:
    - from: "intelligent-work-layer/docs/agent-flows.md (Flow 6)"
      to: "intelligent-work-layer/schemas/briefingschedule-table.json"
      via: "Dataverse List rows query for user schedule"
      pattern: "cr_briefingschedule"
    - from: "intelligent-work-layer/docs/canvas-app-setup.md"
      to: "intelligent-work-layer/schemas/briefingschedule-table.json"
...
@intelligent-work-layer/schemas/senderprofile-table.json
@intelligent-work-layer/docs/agent-flows.md
@intelligent-work-layer/docs/canvas-app-setup.md
@intelligent-work-layer/docs/deployment-guide.md
@intelligent-work-layer/scripts/provision-environment.ps1
...
<interfaces>
From intelligent-work-layer/docs/agent-flows.md — Current Flow 6 trigger (line ~1057-1066):
```markdown
...
From intelligent-work-layer/docs/deployment-guide.md — current BriefingScheduleTime config (line ~303):
```
| `BriefingScheduleTime` | Text | Cron expression for Daily Briefing flow recurrence | `0 7 * * 1-5` (weekdays at 7 AM) |
```

From intelligent-work-layer/schemas/senderprofile-table.json — existing table schema pattern to follow:
The senderprofile-table.json is a good reference for how Dataverse table schemas are documented in this project.

From intelligent-work-layer/src/AssistantDashboard/components/BriefingCard.tsx (line ~126-128):
```typescript
...
<task type="auto">
  ...
  <verify>
    <automated>test -f intelligent-work-layer/schemas/briefingschedule-table.json && grep -c "cr_briefingschedule" intelligent-work-layer/scripts/provision-environment.ps1 && grep -c "BriefingSchedule" intelligent-work-layer/docs/deployment-guide.md</automated>
  </verify>
...
<task type="auto">
  ...
  <verify>
    <automated>test -f intelligent-work-layer/schemas/briefingschedule-table.json && grep -c "BriefingSchedule\|Briefing Schedule" intelligent-work-layer/docs/agent-flows.md && grep -c "BriefingSchedule\|Briefing Schedule" intelligent-work-layer/docs/canvas-app-setup.md</automated>
  </verify>
...
<verification>
1. `cat intelligent-work-layer/schemas/briefingschedule-table.json | python3 -m json.tool > /dev/null` — valid JSON
2. `grep "cr_schedulehour" intelligent-work-layer/schemas/briefingschedule-table.json` — schedule hour column defined
3. `grep "cr_isenabled" intelligent-work-layer/schemas/briefingschedule-table.json` — enabled flag column defined
4. `grep "Briefing Schedule" intelligent-work-layer/docs/agent-flows.md` — Flow 6 references BriefingSchedule table
5. `grep "every 15 minutes" intelligent-work-layer/docs/agent-flows.md` — Flow 6 trigger updated to frequent recurrence
6. `grep "Patch" intelligent-work-layer/docs/canvas-app-setup.md` — Canvas App save formula exists
7. `grep "cr_briefingschedule" intelligent-work-layer/scripts/provision-environment.ps1` — provisioning creates table
8. `grep "BriefingSchedule" intelligent-work-layer/docs/deployment-guide.md` — deployment guide updated
</verification>

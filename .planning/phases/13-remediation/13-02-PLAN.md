...
<task type="auto">
  ...
  <files>intelligent-work-layer/docs/agent-flows.md</files>
  <action>
  ...
  <verify>
    <automated>grep -c "DISMISSED" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: DISMISSED branch" || echo "FAIL"; grep -c "Staleness Monitor\|Flow 8" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: Staleness Monitor" || echo "FAIL"; grep -c "Daily Briefing\|Flow 6" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: Daily Briefing" || echo "FAIL"; grep -c "draft_payload" intelligent-work-layer/docs/agent-flows.md | grep -q '[1-9]' && echo "PASS: envelope wrapping" || echo "FAIL"; grep -c "NUDGE" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: NUDGE spec" || echo "FAIL"</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <verify>
    <automated>grep -c "Command Execution\|Flow 7" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: Command Execution" || echo "FAIL"; grep -c "Sender Profile Analyzer\|Flow 9" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: Sender Profile Analyzer" || echo "FAIL"; grep -c "dismiss_rate" intelligent-work-layer/docs/agent-flows.md | grep -q '[1-9]' && echo "PASS: dismiss_rate logic" || echo "FAIL"; grep -c "COMMAND_TEXT\|commandText" intelligent-work-layer/docs/agent-flows.md | grep -q '[1-9]' && echo "PASS: command text" || echo "FAIL"</automated>
  </verify>
  <done>

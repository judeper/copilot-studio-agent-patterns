...
files_modified:
  ...
  - .planning/PROJECT.md
  - intelligent-work-layer/src/test/ConfidenceCalibration.test.tsx
  - intelligent-work-layer/src/test/index.test.ts
...
must_haves:
  ...
  artifacts:
    - path: "intelligent-work-layer/src/AssistantDashboard/hooks/useCardData.ts"
      provides: "card_status from discrete column via getFormattedValue"
      contains: "getFormattedValue"
    - path: "intelligent-work-layer/src/AssistantDashboard/ControlManifest.Input.xml"
      provides: "orchestratorResponse and isProcessing input properties"
      contains: "orchestratorResponse"
    - path: "intelligent-work-layer/src/AssistantDashboard/components/ErrorBoundary.tsx"
      provides: "React error boundary class component"
      contains: "componentDidCatch"
    - path: "intelligent-work-layer/src/test/ConfidenceCalibration.test.tsx"
      provides: "ConfidenceCalibration test coverage"
      contains: "describe"
    - path: "intelligent-work-layer/src/test/index.test.ts"
...
<task type="auto">
  ...
  <action>
    ...
    **F-03 / I-13: Create React ErrorBoundary**
    - Create new file: intelligent-work-layer/src/AssistantDashboard/components/ErrorBoundary.tsx
  ...
  <verify>
    <automated>grep -c "getFormattedValue" intelligent-work-layer/src/AssistantDashboard/hooks/useCardData.ts | grep -q '[1-9]' && echo "PASS: discrete column read" || echo "FAIL"; grep -c "orchestratorResponse" intelligent-work-layer/src/AssistantDashboard/ControlManifest.Input.xml | grep -q '[1-9]' && echo "PASS: manifest property" || echo "FAIL"; test -f intelligent-work-layer/src/AssistantDashboard/components/ErrorBoundary.tsx && echo "PASS: ErrorBoundary exists" || echo "FAIL"; grep -c "ErrorBoundary" intelligent-work-layer/src/AssistantDashboard/components/App.tsx | grep -q '[1-9]' && echo "PASS: ErrorBoundary used" || echo "FAIL"; grep -c "componentDidCatch" intelligent-work-layer/src/AssistantDashboard/components/ErrorBoundary.tsx | grep -q '[1-9]' && echo "PASS: error boundary class" || echo "FAIL"</automated>
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **I-18: Add monitoring strategy to flow error scopes**
    ...
    - **Add cr_errorlog table to provision-environment.ps1:** After the existing entity creation block (AssistantCard and SenderProfile tables), add a new section that creates the cr_errorlog table with all required columns. Use the same pattern as the existing entity creation (Dataverse Web API POST to EntityDefinitions). PowerShell snippet:
      ```powershell
      # --- Create Error Log table for monitoring (I-18) ---
      $errorLogEntity = @{
        "@odata.type" = "Microsoft.Dynamics.CRM.EntityMetadata"
        SchemaName = "cr_ErrorLog"
        DisplayName = @{ LocalizedLabels = @(@{ Label = "Error Log"; LanguageCode = 1033 }) }
        DisplayCollectionName = @{ LocalizedLabels = @(@{ Label = "Error Logs"; LanguageCode = 1033 }) }
        Description = @{ LocalizedLabels = @(@{ Label = "Flow error monitoring log"; LanguageCode = 1033 }) }
        OwnershipType = "UserOwned"
        HasNotes = $false
        HasActivities = $false
        PrimaryNameAttribute = "cr_flowname"
        Attributes = @(
          @{
            "@odata.type" = "Microsoft.Dynamics.CRM.StringAttributeMetadata"
            SchemaName = "cr_FlowName"
            DisplayName = @{ LocalizedLabels = @(@{ Label = "Flow Name"; LanguageCode = 1033 }) }
            MaxLength = 100
          ...
          @{
            "@odata.type" = "Microsoft.Dynamics.CRM.StringAttributeMetadata"
            SchemaName = "cr_ErrorStep"
            DisplayName = @{ LocalizedLabels = @(@{ Label = "Error Step"; LanguageCode = 1033 }) }
            MaxLength = 200
            AttributeType = "String"
          },
          @{
            "@odata.type" = "Microsoft.Dynamics.CRM.DateTimeAttributeMetadata"
            SchemaName = "cr_OccurredOn"
            DisplayName = @{ LocalizedLabels = @(@{ Label = "Occurred On"; LanguageCode = 1033 }) }
            AttributeType = "DateTime"
            Format = "DateAndTime"
          },
          @{
            "@odata.type" = "Microsoft.Dynamics.CRM.PicklistAttributeMetadata"
            SchemaName = "cr_Severity"
            ...
            OptionSet = @{
              ...
              Options = @(
                @{ Value = 100000000; Label = @{ LocalizedLabels = @(@{ Label = "Info"; LanguageCode = 1033 }) } },
                @{ Value = 100000001; Label = @{ LocalizedLabels = @(@{ Label = "Warning"; LanguageCode = 1033 }) } },
                @{ Value = 100000002; Label = @{ LocalizedLabels = @(@{ Label = "Error"; LanguageCode = 1033 }) } }
              )
            }
          },
          @{
            "@odata.type" = "Microsoft.Dynamics.CRM.BooleanAttributeMetadata"
            SchemaName = "cr_IsResolved"
            DisplayName = @{ LocalizedLabels = @(@{ Label = "Is Resolved"; LanguageCode = 1033 }) }
      ...
      $errorLogJson = $errorLogEntity | ConvertTo-Json -Depth 10
      Invoke-WebRequest -Uri "$envUrl/api/data/v9.2/EntityDefinitions" `
        -Method POST -Headers $headers -Body $errorLogJson -ContentType "application/json"
      Write-Host "Created cr_ErrorLog table for flow error monitoring"
      ```
    - This ensures a freshly provisioned environment has the error log table so flow error scopes can write to it

    **F-04: Create ConfidenceCalibration test file**
    - Create new file: intelligent-work-layer/src/test/ConfidenceCalibration.test.tsx
    ...
    **F-05: Create index.ts PCF lifecycle test file**
    - Create new file: intelligent-work-layer/src/test/index.test.ts
  ...
  <verify>
    <automated>test -f intelligent-work-layer/src/test/ConfidenceCalibration.test.tsx && echo "PASS: ConfidenceCalibration tests exist" || echo "FAIL"; test -f intelligent-work-layer/src/test/index.test.ts && echo "PASS: index tests exist" || echo "FAIL"; grep -c "describe\|it(" intelligent-work-layer/src/test/ConfidenceCalibration.test.tsx 2>/dev/null | grep -q '[5-9]\|[1-9][0-9]' && echo "PASS: multiple test cases" || echo "FAIL"; grep -c "describe\|it(" intelligent-work-layer/src/test/index.test.ts 2>/dev/null | grep -q '[3-9]\|[1-9][0-9]' && echo "PASS: lifecycle tests" || echo "FAIL"; grep -c "cr_errorlog\|Error Monitoring" intelligent-work-layer/docs/agent-flows.md | grep -q '[2-9]' && echo "PASS: monitoring strategy" || echo "FAIL"; grep -c "cr_ErrorLog\|cr_errorlog" intelligent-work-layer/scripts/provision-environment.ps1 | grep -q '[1-9]' && echo "PASS: cr_errorlog in provisioning" || echo "FAIL"</automated>
  </verify>
  <done>
    ...
    - provision-environment.ps1 creates cr_errorlog table with all 7 columns (cr_errorlogid auto, cr_flowname, cr_errormessage, cr_errorstep, cr_occurredon, cr_severity, cr_isresolved)
...
6. All tests pass: `cd intelligent-work-layer/src && npx jest --config test/jest.config.ts`
</verification>

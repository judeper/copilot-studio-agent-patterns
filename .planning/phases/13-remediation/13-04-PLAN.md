...
<task type="auto">
  ...
  <action>
    ...
    **Deferred issues (document with rationale):**
    ...
    - R-17/I-14 (SENDER_PROFILE not passed to agent): Defer -- sender intelligence is partially functional without R-17 because Sender Profile Analyzer (Flow 9) still updates cr_sendercategory, which flows can use for routing decisions. Passing SENDER_PROFILE as an input variable to the main agent requires flow-level input mapping changes in Copilot Studio that need live environment testing to validate. The 12-02 verdict recommended Phase 13 fix, but given the partial functionality and the live-testing dependency, this is deferred to post-deployment when Copilot Studio environment is available for iterative testing.
  ...
  <verify>
    <automated>test -f .planning/phases/13-remediation/13-04-deferral-log.md && echo "PASS: deferral log exists" || echo "FAIL"; grep -c "Deferred" .planning/phases/13-remediation/13-04-deferral-log.md | grep -q '[1-9]' && echo "PASS: deferrals documented" || echo "FAIL"; grep -c "NUDGE" intelligent-work-layer/src/AssistantDashboard/components/CardItem.tsx | grep -q '[1-9]' && echo "PASS: NUDGE in CardItem" || echo "FAIL"; grep -c "SENDER_PROFILE" intelligent-work-layer/docs/deployment-guide.md | grep -q '[1-9]' && echo "PASS: SENDER_PROFILE in guide" || echo "FAIL"; grep -c "PublishAll\|publish" intelligent-work-layer/scripts/provision-environment.ps1 | grep -q '[1-9]' && echo "PASS: publish step" || echo "FAIL"</automated>
  </verify>
  <done>
  ...
  <action>
    **FIX-04: Final validation pass**
    ...
    1. **TypeScript type-check:**
       - Run `cd intelligent-work-layer/src && npx tsc --noEmit` (or the project's configured type-check command)
       - Record pass/fail and any errors

    2. **Test suite:**
       - Run `cd intelligent-work-layer/src && npx jest --config test/jest.config.ts --verbose`
    ...
    4. **Regression check:**
       ...
       - Verify output-schema.json is valid JSON: `node -e "JSON.parse(require('fs').readFileSync('intelligent-work-layer/schemas/output-schema.json', 'utf8'))"`

...
if ($BodyFilePath) {
    ...
    $BodyJson = Get-Content $BodyFilePath -Raw
}
...
if (-not $flow) {
    throw "Flow '$FlowDisplayName' was not found in environment $EnvironmentId."
}
...
$bodyObject = $BodyJson | ConvertFrom-Json
$requestBody = $bodyObject | ConvertTo-Json -Depth 20

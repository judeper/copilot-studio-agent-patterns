...
Parse the sender's email address from the trigger output. The V3 trigger may return `from` as a structured string like `"Sarah Chen" <sarah@example.com>` or just `sarah@example.com`:
